...
suite('Functional Tests', function() {
  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=RAPHAEL') /** <=== Put your name in the query **/ 
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200, 'response status should be 200');
            assert.equal(res.text, 'hello RAPHAEL'/** <==  Put your name here **/);
            done();   // Always call the 'done()' callback when finished.
          });
      });
    ...
    suite('PUT /travellers', function(){
      ...
      test('send {surname: "da Verrazzano"}', function(done) {
          ...
          /** place your tests inside the callback **/
            ...
            assert.equal(res.body.name, 'Giovanni', 'res.name should be Giovanni');
            assert.equal(res.body.surname, 'da Verrazzano', 'res.surname should be da Verrazzano');
            //assert.fail(); // remove this after adding tests
            done();
          });
      });
  ...
  Browser.site = 'https://five-chai.glitch.me/'; 
  ...
  suite('e2e Testing with Zombie.js', function() {
    ...
    suite('"Famous Italian Explorers" form', function() {
      ...
      test('#RAPH - submit "surname" : "Colombo" - write your e2e test...', function(done) {
